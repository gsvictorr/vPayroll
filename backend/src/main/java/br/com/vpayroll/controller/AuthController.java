...
import br.com.vpayroll.service.TokenService;import jakarta.validation.Valid;
...
public class AuthController {
    ...
    @CrossOrigin(origins = "http://localhost:3000/*")
    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody @Valid AuthRequest authRequest) throws LoginException {
        if (authRequest.email().isEmpty() || authRequest.password().isEmpty()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new LoginException("Email ou senha não podem ser vazios."));
        }
